...
Rails (founder-authorized, 2026-07-27):
  ...
  * A machine-generated candidate list (LIVE discovery jobs, deduped by
    employer) is written by `POST /api/v1/prep/dryrun-candidates`. The list
    itself is authorized to READ; execution is founder-only per rails.
"""
...
async def _run(urls: list[str], out_dir: Path, evidence_md: Path, limit: int = 20) -> dict:
    ...
    # Local import so a plain --help doesn't require playwright installed.
    ...
    async with async_playwright() as pw:
        ...
        try:
            context = await browser.new_context(
                user_agent="OpportunityOS-DryRun/1.0 (fixture; contact: support@opportunityos.dev)",
            ...
            for i, url in enumerate(urls[:limit]):
                slot: dict = {"index": i, "url": url, "state": "unknown",
                              "fields_found": 0, "fields_filled": 0,
    ...
    with evidence_md.open("a", encoding="utf-8") as f:
        ...
        for r in results:
            f.write(f"- `{r['url']}` — {r['state']} — "
                    f"fields_found={r['fields_found']} "
                    f"fields_filled={r['fields_filled']} — "
                    f"shot: `{r['screenshot'] or 'n/a'}`\n")
    return summary
